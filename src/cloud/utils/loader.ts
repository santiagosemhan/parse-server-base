...
function loadCloudFunctions(legacy: boolean = false) {
  const cloudFunctions: Map<string, Sensbox.RouteDefinition> = new Map();
  Object.keys(routes).forEach((controller: string) => {
    ...
    Object.keys(routes[controller]).forEach((action: string) => {
      ...
      if (cloudFunctions.has(actionPath)) {
        throw new Error(
          `${actionPath} was already defined. Please check your routes` +
            'definitions in your route directory.',
        );
      }
      // @ts-ignore
  ...
  cloudFunctions.forEach((cloudFnDefinition: Sensbox.RouteDefinition, path: string): void => {
    const { action, secure: isSecure, onlyMaster } = cloudFnDefinition;
    if (isSecure) {
      Parse.Cloud.define(path, secure(action, onlyMaster));
    } else {
      Parse.Cloud.define(path, <Sensbox.CloudFunction>action);
    }
    console.info('Loaded ', path, `${onlyMaster ? '(master)' : ''}`);
  });
}
